...
directions.on("route", (event) => {
  ...
  if (counter >= maxAttempts) {
    ...
    for (const route of event.route) {
      ...
      let totalObstaculoRota = 0;
      for (const obstacle of obstacles) {
        const clear = turf.booleanDisjoint(obstacle, routeLine);
        const totalIntersections = turf.lineIntersect(obstacle, routeLine).features.length;
        const totalIntersectionsObstacle = (totalIntersections === 2) ? totalIntersections / 2 : totalIntersections;
        totalObstaculoRota += totalIntersectionsObstacle;
      }
    
      console.log("🚀 ~ file: main.js:187 ~ directions.on ~ totalObstaculoRota:", totalObstaculoRota)
