...
directions.on('route', (event) => {
  ...
  if (counter >= maxAttempts) {
    ...
    // Initialize minimum obstacle count to a high value
let minObstacleCount = Infinity;
let minObstacleRoute = null;
...
for (const route of event.route) {
  ...
  const obstacleCount = turf.lineIntersect(obstacle, routeLine).features.length / 2;
  const clear = obstacleCount === 0;

  if (clear) {
    // Route is clear of obstacles
    ...
    collision = 'is bad.';
    detail = `takes ${(route.duration / 60).toFixed(0)} minutes and hits ${obstacleCount} obstacles`;
    emoji = '⚠️';
    ...
    // Check if this route has the least obstacles encountered so far
    if (obstacleCount < minObstacleCount) {
      minObstacleCount = obstacleCount;
      minObstacleRoute = route;
    }
  }
...
// Display only the route with the least obstacles encountered
if (minObstacleRoute) {
  const minObstacleLine = polyline.toGeoJSON(minObstacleRoute.geometry);
  map.getSource('theRoute').setData(minObstacleLine);
  map.setPaintProperty('theRoute', 'line-color', '#74c476');
  map.setLayoutProperty('theBox', 'visibility', 'none');
}
