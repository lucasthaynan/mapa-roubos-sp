      ...
      directions.on('route', (event) => {
        ...
        if (counter >= maxAttempts) {
          ...
          for (const route of event.route) {
            ...
            detail = `${numberOfObstacles} Obs - leva ${(route.duration / 60).toFixed(0)} minutes and avoids`;
            emoji = '✔️';
            ...
            collision = 'is bad.';
            detail = `${numberOfObstacles} Obs - takes ${(route.duration / 60).toFixed(0)} minutes and hits`;
            emoji = '⚠️';
            map.setPaintProperty('theRoute', 'line-color', '#de2d26');

            // Count the number of obstacles
            numberOfObstacles = route.legs.reduce((acc, leg) => {
                return acc + leg.annotation?.distance.filter((d) => d === -1).length || 0;
            }, 0);
            ...
            addCard(counter, reports, clear, detail, numberOfObstacles);
