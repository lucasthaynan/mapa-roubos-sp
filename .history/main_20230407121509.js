...
directions.on("route", (event) => {
  ...
  if (counter >= maxAttempts) {
    ...
    for (const route of event.route) {
      ...
      // Find overlapping obstacles
      const intersectingObstacles = event.obstacles.filter((obstacle) => {
        return !turf.booleanDisjoint(obstacle, routeLine);
      });

      // Count the number of overlapping obstacles
      let overlappingObstaclesCount = 0;
      intersectingObstacles.forEach((obstacle) => {
        overlappingObstaclesCount += turf.lineIntersect(obstacle, routeLine).features.length / 2;
      });

      totalObstaculoRota = overlappingObstaclesCount;

      routesInfo[idRota] = {
        routeLine: routeLine,
        bbox: bbox,
        polygon: polygon,
        clear: totalObstaculoRota === 0,
        obstacles: totalObstaculoRota,
      };

      idRota += 1

      if (totalObstaculoRota === 0) {
        ...
        collision = "is bad.";
        detail = `takes ${(route.duration / 60).toFixed(0)} minutes and hits ${totalObstaculoRota} obstacles`;
      ...
      addCard(counter, reports, totalObstaculoRota === 0, detail);
    }
  }
});
