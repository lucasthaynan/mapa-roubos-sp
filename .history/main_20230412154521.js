...
function zoomToRoute(map, directions) {
  directions.on("route", function (event) {
    if (event.route.length > 0) {
      // Obtem a bounding box da rota gerada
      const bbox = turf.bbox(event.route[0].geometry);
...
function removeRoutes(map) {
  ...
  if (map.getSource("bestRoute2")) {
    map.removeLayer("bestRoute2");
    map.removeSource("bestRoute2");
  }
  if (map.getSource("bestRoute")) {
