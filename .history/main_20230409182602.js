...
const source = map.getSource("theBox");
if (source) {
  ...
  if (counter >= maxAttempts) {
    ...
    for (const route of event.route) {
      ...
      totalObstaculoRota =
        ...
        map.getSource('theRoute' + idRota).setData(routeLine).setProperties({id: idRota});
        console.log("🚀 ~ file: main.js:195 ~ directions.on ~ idRota:", idRota)
        console.log("🚀 ~ file: main.js:195 ~ directions.on ~ map:", map)
...
function removeRoute(idRota) {
  const routeLayerName = 'theRoute' + idRota;
  const routeSourceName = 'theRoute' + idRota;
