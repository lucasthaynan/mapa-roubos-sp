...
fetch("./clearance_map.json")
  ...
  .then((data) => {
    // Adiciona um pequeno offset às coordenadas de cada obstáculo
    const featuresWithOffset = data.features.map((feature) => {
      const [lng, lat] = feature.geometry.coordinates;
      const offset = Math.random() * 0.0002 - 0.0001; // valor aleatório entre -0.0001 e 0.0001
    ...
    // Cria o buffer com uma distância de 0.01 km em torno de cada obstáculo
    obstacle = turf.buffer(featuresWithOffset, 0.01, { units: "kilometers" });

    // Use o obstáculo criado acima com Mapbox
    // ...
  })
