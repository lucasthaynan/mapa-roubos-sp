...
// Array para armazenar o número de obstáculos de cada rota
const obstacleCount = [];
...
directions.on('route', (event) => {
  ...
  if (counter >= maxAttempts) {
    ...
    for (const route of event.route) {
      ...
      if (clear === true) {
        ...
        detail = `${counter} obstáculos - leva ${(route.duration / 60).toFixed(
          0
        )} minutes and avoids`;
        emoji = '✔️';
        ...
        // Adicione o número de obstáculos ao array obstacleCount
        obstacleCount.push(counter);
        ...
        detail = `takes ${(route.duration / 60).toFixed(
          0
        )} minutes and hits ${turf.lineIntersect(obstacle, routeLine).features.length} obstacles`;
        emoji = '⚠️';
  ...
  // Se tivermos tentado encontrar rotas suficientes e ainda não encontrarmos uma rota clara,
  // encontre a rota com o menor número de obstáculos e destaque-a no mapa
  if (counter >= maxAttempts) {
    const minObstacleCount = Math.min(...obstacleCount);
    for (let i = 0; i < obstacleCount.length; i++) {
      if (obstacleCount[i] === minObstacleCount) {
        const routeLine = polyline.toGeoJSON(event.route[i].geometry);
        map.getSource('theRoute').setData(routeLine);
        map.setPaintProperty('theRoute', 'line-color', '#74c476');
        break;
      }
