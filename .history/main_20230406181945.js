...
directions.on("route", (event) => {
  ...
  if (counter >= maxAttempts) {
    ...
    for (const route of event.route) {
      ...
      // Adiciona o número de obstáculos da rota no array
      totalObstaculoRotas.push(totalObstaculoRota);

      idRota += 1
    }

    // Obtém o menor valor do array de obstáculos
    const menorObstaculos = Math.min(...totalObstaculoRotas);

    let bestRoute;

    // Percorre novamente as rotas e armazena as informações da rota com menos obstáculos
    for (const [id, routeInfo] of Object.entries(routesInfo)) {
      if (routeInfo.obstacles === menorObstaculos) {
        bestRoute = routeInfo;
        break;
      }
    }

    // Define a melhor rota como a rota atual
    map.getSource("theRoute").setData(bestRoute.routeLine);
    map.getSource("theBox").setData(bestRoute.polygon);
    if (bestRoute.clear === true) {
      ...
      collision = "is bad.";
      detail = `takes ${(event.route[0].duration / 60).toFixed(0)} minutes and hits ${
        bestRoute.obstacles
      } obstacles`;
