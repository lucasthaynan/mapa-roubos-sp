...
fetch("./clearance_map.json")
  ...
  .then((data) => {
    ...
    // Adicionar deslocamento aleatório às coordenadas dos obstáculos
    const obstacleFeatures = clearances.features.map((obstacle) => {
      const displacement = Math.random() * 0.001; // Valor aleatório entre 0 e 0.001
      const coordinates = obstacle.geometry.coordinates;
      const displacedCoordinates = [coordinates[0] + displacement, coordinates[1] + displacement];
      const displacedGeometry = turf.point(displacedCoordinates);
      return turf.feature(displacedGeometry, obstacle.properties);
    });

    // Criar uma coleção de features com os obstáculos deslocados
    const obstacleCollection = turf.featureCollection(obstacleFeatures);

    // Criar o buffer a partir da coleção de features com os obstáculos deslocados
    obstacle = turf.buffer(obstacleCollection, 0.01, { units: "kilometers" });
