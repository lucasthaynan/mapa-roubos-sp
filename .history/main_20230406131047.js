...
directions.on('route', (event) => {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    for (const route of routes) {
      ...
      let obstacleClearanceSum = 0; // Initialize variable to store the total clearance of all obstacles for this route
      ...
      if (intersection.features.length > 0) { // If there's at least one obstacle, add to counter and array
        intersection.features.forEach((feature) => {
          obstacleCount++;
          obstacleClearanceSum += clearances.features.find((c) => {
            return c.geometry.coordinates[0] === feature.geometry.coordinates[0] && 
                   c.geometry.coordinates[1] === feature.geometry.coordinates[1];
      ...
      heading.innerHTML = `${emoji} Route ${route.id + 1} ${collision} <span class="obstacle-count"></span> <span class="obstacle-clearance-sum"></span>`;
    
      const obstacleCountElement = heading.querySelector('.obstacle-count');
      obstacleCountElement.innerText = obstacleCount;
    
      const obstacleClearanceSumElement = heading.querySelector('.obstacle-clearance-sum');
      obstacleClearanceSumElement.innerText = obstacleClearanceSum > 0 ? `(Total clearance: ${obstacleClearanceSum})` : '';
    
      if (obstacleCount > 0) {
        const totalClearance = intersection.features.reduce((sum, feature) => {
          const obstacleClearance = clearances.features.find(clearanceFeature =>
            turf.booleanPointInPolygon(feature.geometry, clearanceFeature.geometry)
          ).properties.clearance;
          return sum + obstacleClearance;
        }, 0);
        ...
        const obstacleWeight = totalClearance / obstacleCount;
        const weightText = `Average obstacle weight: ${obstacleWeight.toFixed(2)}`;
    
        const weight = report.appendChild(document.createElement('p'));
        weight.innerHTML = weightText;
    
        const details = report.appendChild(document.createElement('div'));
        details.innerHTML = `This route goes through ${obstacleCount} avoidance area(s) with a total clearance of ${totalClearance.toFixed(2)}.`;
        report.appendChild(document.createElement('hr'));
    
        if (obstacleWeight > 6) {
          map.setPaintProperty(`route${route.id}`, 'line-color', '#de2d26');
          weight.className = 'warning';
          details.innerHTML += ` This route has a high average obstacle weight of ${obstacleWeight.toFixed(2)}.`;
        } else {
          map.setPaintProperty(`route${route.id}`, 'line-color', '#74c476');
        }
      } else {
        ...
        heading.innerHTML = `✔️ Route ${route.id + 1} is good!`;
    
        const details = report.appendChild(document.createElement('div'));
        details.innerHTML = `This route does not go through any avoidance areas.`;
        report.appendChild(document.createElement('hr'));
      }
