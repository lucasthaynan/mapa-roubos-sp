...
fetch("./clearance_map.json")
  ...
  .then((data) => {
    ...
    // Gerar uma grade de pontos para deslocar obstáculos sobrepostos
    const gridSize = 0.001; // Distância entre os pontos da grade
    const bbox = turf.bbox(obstacle);
    const points = turf.pointGrid(bbox, gridSize, { units: "kilometers" });

    // Deslocar obstáculos sobrepostos
    const overlaps = turf.booleanOverlap(obstacle);
    overlaps.features.forEach((overlap) => {
      const id1 = overlap.properties.featureIndex;
      const id2 = overlap.properties.index;
      const displacement = [0, 0.001]; // Deslocamento na coordenada y
      obstacle.features[id2] = turf.displace(obstacle.features[id2], displacement);
    });
