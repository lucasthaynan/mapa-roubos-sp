  ...
  if (counter >= maxAttempts) {
    ...
    for (const route of event.route) {
      ...
      if (clear === true) {
        ...
        console.log(
          "🚀 ~ file: main.js:284 ~ directions.on ~ turf.lineIntersect(obstacle, routeLine):",
          turf.lineIntersect(obstacle, routeLine)
        );
      ...
      // Adiciona as informações da rota no objeto routesInfo
      addCard(counter, reports, clear, detail);
    }

    // adiciona o trecho abaixo para encontrar a rota com menos obstáculos
    const sortedRoutes = Object.entries(routesInfo).sort(([, a], [, b]) => {
      return a.obstacles - b.obstacles;
    });
    const bestRouteId = sortedRoutes[0][0];
    const bestRouteInfo = routesInfo[bestRouteId];
    map.getSource("theRoute").setData(bestRouteInfo.routeLine);
    map.getSource("theBox").setData(bestRouteInfo.polygon);
  }
});
