...
directions.on("route", (event) => {
  if (counter >= maxAttempts) {
    ...
    for (const route of event.route) {
      ...
      const routeName = clear ? "bestRoute" : `rota-${idRota}`;
    ...
    if (counter >= maxAttempts) {
      ...
      for (const id in routesInfo) {
        const { name, obstacles, routeLine } = routesInfo[id];
        ...
        // set the route layer ID based on the route name
        map.getSource(name).setData(routeLine);
      }
      ...
      if (minObstacles === maxObstacles) {
        map.setPaintProperty("bestRoute", "line-color", "#9370db");
        map.setLayoutProperty("theBox", "visibility", "none");
        routeLayerId = "bestRoute"; // assign ID to the route layer
      } else {
        map.setPaintProperty("worstRoute", "line-color", "#74c476");
        map.setLayoutProperty("theBox", "visibility", "none");
