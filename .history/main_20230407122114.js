...
directions.on("route", (event) => {
  ...
  if (counter >= maxAttempts) {
    ...
    for (const route of event.route) {
      ...
      // Count the number of overlapping obstacles with the route line
      let overlappingObstacles = 0;
      for (const obstacleFeature of obstacle.features) {
        if (turf.booleanOverlap(obstacleFeature, routeLine)) {
          overlappingObstacles += 1;
        }
      }
      ...
      routesInfo[idRota] = {
        ...
        bbox: bbox,
        polygon: polygon,
        clear: overlappingObstacles === 0,
        obstacles: overlappingObstacles,
      };
      ...
      addCard(counter, reports, overlappingObstacles === 0, detail);
