...
function removeRoutes() {
  ...
  map.getStyle().layers.forEach(function (layer) {
    if (layer.id !== 'background' && layer.id.indexOf('theRoute') !== -1) {
...
function removeRoutes3() {
  var layersToRemove = [];
  map.getStyle().layers.forEach(function (layer) {
    if (layer.id !== 'background' && layer.source === 'theRoute') {
      layersToRemove.push(layer.id);
    }
  });
...
function removeMapboxRoutes(map) {
  // Remove a camada de rotas
  map.removeLayer('theRoute');

  // Remove a fonte de dados das rotas
  map.removeSource('theRoute');
}


function removeRoutes4() {
