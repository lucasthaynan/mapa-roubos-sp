...
directions.on("route", (event) => {
  ...
  if (counter >= maxAttempts) {
    ...
    // Combine all obstacles into a single MultiPolygon feature
const allObstacles = turf.combine(event.obstacles);

// Extract individual polygons from the MultiPolygon feature
const obstacles = turf.flatten(allObstacles);
...
for (const route of event.route) {
  ...
  // Check if any obstacle intersects the route
  let clear = true;
  let totalObstaculoRota = 0;
  for (const obstacle of obstacles.features) {
    if (!turf.booleanDisjoint(obstacle, routeLine)) {
      clear = false;
