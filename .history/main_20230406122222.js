...
directions.on('route', (event) => {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    for (const route of routes) {
      ...
      // Update the obstacle count element
  ...
  // Add a final report with the total obstacle count
  const totalObstacleCount = routes.reduce((acc, route) => acc + route.obstacleCount, 0);
  const obstacleReport = reports.appendChild(document.createElement('div'));
  obstacleReport.className = 'item';
  obstacleReport.innerHTML = `There are ${totalObstacleCount} obstacle(s) in the path.`;
});
