...
directions.on('route', (event) => {
  ...
  let totalClearance = 0; // Initialize variable to store total clearance of all obstacles on this route
  ...
  if (intersection.features.length > 0) { // If there's at least one obstacle, add to counter and array
    intersection.features.forEach((feature) => {
      const clearance = feature.properties.clearance;
  ...
  const avgClearance = obstacleCount > 0 ? totalClearance / obstacleCount : 0;
  ...
  const warning = avgClearance > 5 ? 'with caution' : 'at your own risk';
