      ...
      // Define as coordenadas dos pontos que representam os obstáculos e a folga de distância em metros
const clearances = {
    ...
    features: [
      {
        ...
        properties: {
          clearance: 10 // Folga de 10 metros
        }
      },
      {
        type: 'Feature',
        ...
        properties: {
          clearance: 20 // Folga de 20 metros
        }
      }
    ]
  };
  
  // Cria uma zona de buffer em torno dos obstáculos, com uma distância igual à folga definida
  const obstacle = turf.buffer(clearances, 0.1, { units: 'kilometers' });
  
  // Define uma bounding box vazia
  let bbox = [0, 0, 0, 0];
  
  // Cria um polígono a partir da bounding box vazia
  let polygon = turf.bboxPolygon(bbox);
  
  // Quando o mapa estiver pronto
  map.on('load', () => {
    // Adiciona uma camada que representa as zonas de buffer dos obstáculos
    map.addLayer({
      id: 'clearances',
      type: 'fill',
      source: {
    ...
    // Adiciona uma fonte de dados vazia para a rota traçada
    ...
    // Adiciona uma camada que representa a rota traçada pelo usuário
    map.addLayer({
      id: 'theRoute',
      type: 'line',
      source: 'theRoute',
