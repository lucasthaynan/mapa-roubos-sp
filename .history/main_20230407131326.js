...
directions.on("route", (event) => {
  ...
  const obstacleFeatures = obstacle.features;
  const obstaclesByCoordinate = {};
  for (const obstacleFeature of obstacleFeatures) {
    const coordinates = obstacleFeature.geometry.coordinates;
    const coordinateKey = coordinates.join(',');
    if (!obstaclesByCoordinate[coordinateKey]) {
      obstaclesByCoordinate[coordinateKey] = {
        coordinates,
        count: 0,
      };
    }
    obstaclesByCoordinate[coordinateKey].count += 1;
  }
  ...
  if (counter >= maxAttempts) {
    ...
    for (const route of event.route) {
      ...
      totalObstaculoRota =
        ...
        let totalObstaculos = [];
        const overlappingObstacleFeatures = turf.booleanOverlap(obstacle, routeLine).features;
        for (const overlappingObstacleFeature of overlappingObstacleFeatures) {
          const overlappingObstacleCoordinates = overlappingObstacleFeature.geometry.coordinates;
          const overlappingObstacleKey = overlappingObstacleCoordinates.join(',');
          if (obstaclesByCoordinate[overlappingObstacleKey]) {
            totalObstaculos.push(obstaclesByCoordinate[overlappingObstacleKey]);
          }
        }

      // // Find all overlapping obstacles
      // const overlappingObstacles = obstacles.features.filter((obstacle) =>
      //   turf.booleanOverlap(obstacle, routeLine)
      // );

      // let totalObstacles = 0;
      // for (const overlappingObstacle of overlappingObstacles) {
      //   totalObstacles += turf.lineIntersect(overlappingObstacle, routeLine)
      //     .features.length;
      // }
      // // Divide by 2 since lineIntersect returns 2 points per intersection
      // totalObstacles /= 2;
      ...
      routesInfo[idRota] = {
        ...
        bbox: bbox,
        polygon: polygon,
        clear: totalObstaculos.length === 0,
        obstacles: totalObstaculos,
      };
