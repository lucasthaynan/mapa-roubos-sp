  ...
  if (counter >= maxAttempts) {
    ...
    for (const route of event.route) {
      ...
      if (bestRoute) {
        ...
        console.log("🚀 ~ file: main.js:331 ~ directions.on ~ routeLine:", routeLine)
        console.log(turf.lineIntersect(obstacle, routeLine))
        bbox = turf.bbox(routeLine);
        polygon = turf.bboxPolygon(bbox);
        map.getSource('theRoute').setData(routeLine);
        map.getSource('theBox').setData(polygon);
      ...
      if (clear === true) {
        ...
        detail = `takes ${(route.duration / 60).toFixed(
          0
        )} minutes and hits ${(turf.lineIntersect(obstacle, routeLine).features.length)/2} obstacles`;
        console.log("🚀 ~ file: main.js:284 ~ directions.on ~ turf.lineIntersect(obstacle, routeLine):", turf.lineIntersect(obstacle, routeLine))
