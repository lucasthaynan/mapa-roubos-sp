...
function noRoutes(element) {
  ...
  details.innerHTML = `Nenhuma rota sem assaltos foi encontrada no trajeto, em ${counter} tentativas. 
    <br><br> A melhor rota foi a que teve ${minimoAssaltosRota} registros, ela registrou ⬇ ${percentualMinObstacles}% assaltos a menos em relação à média das rotas verificadas.`;
...
directions.on("route", (event) => {
  if (counter >= maxAttempts) {
    ...
    if (counter >= maxAttempts) {
      // Limpar a última rota testada (em azul)
      map.getSource("theRoute").setData({ type: "FeatureCollection", features: [] });
