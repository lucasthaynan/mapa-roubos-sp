...
function removeRoutes(map) {
  // Obter todas as camadas no mapa
  const layers = map.getStyle().layers;

  // Iterar sobre as camadas
  for (const layer of layers) {
    // Verificar se a camada é uma rota (ajuste essa condição conforme necessário)
    if (layer.id.includes("theRoute")) {
      // Remover a camada e a fonte de dados relacionada
      map.removeLayer(layer.id);
      map.removeSource(layer.id);
    } else {
      map.removeLayer("teste");
      map.removeSource("teste");
    }
  }
}
