...
directions.on("route", (event) => {
  ...
  if (counter >= maxAttempts) {
    ...
    for (const route of event.route) {
      ...
      const obstacles = turf.lineIntersect(obstacle, routeLine).features.length / 2;
      ...
      if (clear) {
        ...
        const randomWaypoint = turf.randomPoint(1, { bbox });
      ...
      const detail = `takes ${(route.duration / 60).toFixed(0)} minutes and hits ${obstacles} obstacles`;
