...
function btnLimparRotaAzul() {
  ...
  rotasTracadas.forEach(rota => {
    ...
    if (rota.name.endsWith("bestRoute")) {
...
directions.on("route", (event) => {
  ...
  const routeOrigemDestino = event.route[0];
  if (route && routeOrigemDestino.origin && routeOrigemDestino.origin.geometry) {
    origin = route.origin.geometry.coordinates;
  }
  if (route && routeOrigemDestino.destination && routeOrigemDestino.destination.geometry) {
    destination = route.destination.geometry.coordinates;
  }
