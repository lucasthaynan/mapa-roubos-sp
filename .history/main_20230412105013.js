...
// Adiciona um listener para o evento 'waypoint'
map.directions.on('waypointssave', function(e) {
  console.log('Waypoints alterados: ', e.waypoints);
});
...
directions.on("origin", (origin) => {
  ...
  if (origem === null || !novaOrigem.equals(origem)) {
    origem = novaOrigem;
...
directions.on("destination", (destination) => {
  ...
  if (destino === null || !novoDestino.equals(destino)) {
    destino = novoDestino;
