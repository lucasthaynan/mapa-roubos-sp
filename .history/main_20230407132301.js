...
directions.on("route", (event) => {
  ...
  if (counter >= maxAttempts) {
    ...
    for (const route of event.route) {
      ...
      const overlap = turf.booleanOverlap(obstacle, routeLine);
      
      let totalObstaculoRota = 0;
      if (overlap) {
        const obstacles = turf.collect(obstacle, routeLine, 'rheoCoord');
        totalObstaculoRota = Object.keys(obstacles).length;
      }
      ...
      addCard(counter, reports, !overlap, detail);
