...
fetch("./clearance_map.json")
  ...
  .then((data) => {
    // Verificar se há obstáculos com as mesmas coordenadas geográficas e separá-los
    const uniqueCoords = [];
    const uniqueClearances = [];
    for (let i = 0; i < data.features.length; i++) {
      const coords = data.features[i].geometry.coordinates.toString();
      if (!uniqueCoords.includes(coords)) {
        uniqueCoords.push(coords);
        uniqueClearances.push(data.features[i]);
      } else {
        const idx = uniqueCoords.indexOf(coords);
        const separatedObstacle = turf.transformTranslate(
          data.features[i],
          0.0001 * (idx + 1),
          0.0001 * (idx + 1)
        );
        uniqueClearances[idx] = turf.union(
          uniqueClearances[idx],
          separatedObstacle
        );
      }
    }
    obstacle = turf.buffer(turf.featureCollection(uniqueClearances), 0.01, {
      units: "kilometers",
    });
