  ...
  if (counter >= maxAttempts) {
    ...
    for (const route of event.route) {
      ...
      if (totalObstaculoRota < minObstacles) {
        ...
        bestRouteInstructions = [];
        for (const leg of route.legs) {
          for (const step of leg.steps) {
            bestRouteInstructions.push(step.maneuver.instruction);
          }
        }
      }
      // Use bestRouteInstructions to display street-by-street instructions for the best route
      console.log("Best route instructions:", bestRouteInstructions);
...
function updateSidebar(routeInfo, map) {
  ...
  for (let i = 0; i < routeInfo.instructions.length; i++) {
    const instruction = routeInfo.instructions[i];
    const item = document.createElement("li");
    item.textContent = instruction;
    item.addEventListener("click", () => {
      // const map = mapboxgl.Map({});
      const [lng, lat] = routeInfo.polygon.geometry.coordinates[i];
...
// Lucas Thaynan
// Ajustar codigo para adicionar em routesInfo[idRota] o nome da rota, como bestRoute e worstRoute:
